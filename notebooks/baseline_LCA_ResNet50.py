...
class ResNet_50_Backbone(nn.Module):
...
for fold, (train_idx, val_idx) in enumerate(sgkf.split(df_train, df_train['label'], groups)):
    ...
    # === init model ===
    model = ResNet_50_Backbone(img_size=input_img_shape, out_classes=2, freeze_backbone=False).to(DEVICE)
