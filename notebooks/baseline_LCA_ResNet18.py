...
model_name = "resnet_18_backbone_cv" # model train name
prefix_train = "full_tuning_patient" # prefix train
...
class ResNet_18_Backbone(nn.Module):
    def __init__(self, img_size: tuple[int, int, int], out_classes: int, freeze_backbone: bool = True):
        ...
        self.backbone.fc = nn.Linear(num_features, self.out_classes)
...
for fold, (train_idx, val_idx) in enumerate(sgkf.split(df_train, df_train['label'], groups)):
    ...
    # === init model ===
    model = ResNet_18_Backbone(img_size=input_img_shape, out_classes=2, freeze_backbone=False).to(DEVICE)
...
path2save = '..\\models_checkpoints/resnet_18_backbone_cv/2026-04-23_18_17_44_full_tuning_patient'

# '..\\models_checkpoints/resnet_18_backbone_cv/2026-04-23_18_17_44_full_tuning_patient'
...
# === model init ===
metrics_per_folds = dict()

for i in range(CV_N):
    fold = f"fold{i+1}"
    
    path2file_fold = Path(path2save) / fold / "train_history.json"

    with open(path2file_fold, "r", encoding="utf-8") as file:
        json_file = json.loads(file.read())
    

    item_accuracy_mean = np.mean(json_file["accuracy"])    
    item_balanced_accuracy_mean = np.mean(json_file["balanced_accuracy"])
    item_f1_mean = np.mean(json_file["f1"])
    
    item_accuracy_std = np.std(json_file["accuracy"])    
    item_balanced_accuracy_std = np.std(json_file["balanced_accuracy"])
    item_f1_std = np.std(json_file["f1"])

    metrics_per_folds[fold] = {
...
for metric in ['accuracy', 'balanced_accuracy', 'f1']:
    values = [metrics_per_folds[f][metric] for f in metrics_per_folds.keys()]
